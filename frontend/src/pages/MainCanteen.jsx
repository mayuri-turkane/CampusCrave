...
const categoryNames = {
  ...
  quick_fix: "Crispy Bites & Quick Fix",
  sip_chill: "Sip & Chill",
...
function MainCanteen({ cart, setCart }) {
  ...
  return (
    <div className="bg-white min-h-screen pb-40 font-sans selection:bg-orange-100">
      ...
      {/* --- HERO HEADER (Matches Hangout) --- */}
      <div className="relative h-64 md:h-80 w-full overflow-hidden">
        <img
          src="https://images.unsplash.com/photo-1546833998-877b37c2e5c6?auto=format&fit=crop&w=1200&q=80"
        ...
        <div className="absolute inset-0 flex flex-col justify-end p-6 md:p-12 bg-gradient-to-t from-black/80 to-transparent">
          ...
          <div className="max-w-6xl mx-auto w-full">
            <h1 className="text-4xl md:text-6xl font-black text-white tracking-tighter">
              Main Canteen <span className="text-orange-400">🍛</span>
            </h1>
            <div className="flex items-center gap-4 mt-3">
              <span className="flex items-center gap-1 bg-green-500 text-white px-2 py-0.5 rounded-lg text-xs font-bold"><Star size={14} fill="currentColor" /> 4.2</span>
              <span className="text-gray-300 font-medium text-sm flex items-center gap-1"><Clock size={14} /> 15 MINS</span>
            </div>
          </div>
        </div>
      </div>

      {/* --- SEARCH & CATEGORIES (Standardized Sticky) --- */}
          ...
          <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
              ...
              {Object.keys(menuData).map((catKey) => (
                <button
                  ...
                  className={`px-5 py-2 rounded-2xl font-black text-xs uppercase tracking-widest whitespace-nowrap transition-all ${
                    activeCategory === catKey ? "bg-orange-500 text-white shadow-lg shadow-orange-200" : "bg-gray-100 text-gray-500 hover:bg-gray-200"
                  }`}
                >
            ...
            <div className="relative w-full md:w-72">
              ...
              <input
                ...
                className="w-full bg-gray-50 border-none rounded-2xl pl-12 pr-4 py-3 text-sm focus:ring-2 focus:ring-orange-400 transition-all outline-none"
      ...
      <div className="max-w-6xl mx-auto px-6 mt-10">
        <div className="flex items-center gap-3 mb-8">
          ...
          <h2 className="text-2xl font-black text-gray-900 tracking-tight">Top Picks in {categoryNames[activeCategory]}</h2>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {(menuData[activeCategory] || [])
            ...
            .map((item) => {
              ...
              return (
                <div key={item.id} className="flex justify-between items-start bg-white p-4 md:p-6 rounded-[2.5rem] border border-gray-100 hover:shadow-2xl hover:shadow-gray-200 transition-all group">
                  <div className="flex-1 pr-6">
                    ...
                    <h3 className="text-xl font-black text-gray-800 group-hover:text-orange-500 transition-colors">
                      {item.name}
                  ...
                  <div className="relative">
                    <div className="w-32 h-32 rounded-3xl overflow-hidden shadow-inner bg-gray-100">
                      <img
                        src={`http://localhost:5000/static/images/${item.image_url}`}
                        alt={item.name}
                    ...
                    <div className="absolute -bottom-3 left-1/2 -translate-x-1/2 w-28">
                      {cartItem ? (
                        <div className="flex items-center justify-between bg-white text-orange-600 rounded-xl shadow-xl border border-orange-100 overflow-hidden px-1 py-1">
                          <button onClick={() => removeItem(item)} className="p-2 hover:bg-orange-50 rounded-lg transition-colors"><Minus size={16} strokeWidth={3} /></button>
                          <span className="font-black text-sm">{cartItem.qty}</span>
                          <button onClick={() => addItem(item)} className="p-2 hover:bg-orange-50 rounded-lg transition-colors"><Plus size={16} strokeWidth={3} /></button>
                        </div>
                      ) : (
                        <button
                          onClick={() => addItem(item)}
                          className="w-full bg-white text-orange-600 border border-orange-200 py-2.5 rounded-xl font-black text-xs uppercase tracking-widest shadow-xl hover:bg-orange-500 hover:text-white transition-all transform active:scale-90"
                        >
                          Add
                        </button>
                      )}
      ...
      {/* --- FLOATING CART --- */}
      {cart.length > 0 && (
        <button onClick={() => navigate("/cart")} className="fixed bottom-8 right-8 bg-orange-500 text-white p-5 rounded-full shadow-2xl z-50 flex items-center gap-3 hover:scale-110 transition-all active:scale-95 group">
          <div className="relative">
